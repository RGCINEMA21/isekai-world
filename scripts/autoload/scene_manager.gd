...
func change_scene(scene_name: String, _data: Dictionary = {}) -> void:
	if is_transitioning:
		return
	...
	if not ResourceLoader.exists(scene_path):
		push_error("[SceneManager] Scene file missing: %s" % scene_path)
		return
	...
	if error != OK:
		push_error("[SceneManager] Failed to change scene: %s (err %d)" % [scene_name, error])
